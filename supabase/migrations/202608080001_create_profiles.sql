...
create table if not exists public.profiles (
  ...
  photo_url text,
  photo_zoom numeric(3,2) not null default 1 constraint profiles_photo_zoom_range check (photo_zoom between 1 and 2),
  photo_position_x smallint not null default 50 constraint profiles_photo_position_x_range check (photo_position_x between 0 and 100),
  photo_position_y smallint not null default 50 constraint profiles_photo_position_y_range check (photo_position_y between 0 and 100),
  cover_url text,
  cover_zoom numeric(3,2) not null default 1 constraint profiles_cover_zoom_range check (cover_zoom between 1 and 2),
  cover_position_x smallint not null default 50 constraint profiles_cover_position_x_range check (cover_position_x between 0 and 100),
  cover_position_y smallint not null default 50 constraint profiles_cover_position_y_range check (cover_position_y between 0 and 100),
